...
*    一般情况下，浏览器的layout是lazy的，在执行js脚本时，是不会去更新DOM的，任何对DOM的修改都会被暂存在一个队列中，在当前js的执行上下文完成执行后，会根据这个队列
*    中的修改，进行一次layout,当需要在js中立刻取得最新的dom节点信息，浏览器就不得不提前执行layout,这是导致DOM性能的主要原因。
*    如下操作会触发浏览器执行layout:
*      (1)通过js获取需要计算的DOM属性      clientHeight这个属性是需要计算得到的，若获取该属性就会触发浏览器的一次layout
*      (2)添加或删除DOM元素
*      (3)resize浏览器窗口的大小
*      (4)改变字体
*      (5)css伪类的激活
*      (6)通过js修改DOM元素的样式且涉及到尺寸变化
