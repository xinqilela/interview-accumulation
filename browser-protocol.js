...
* 2.查找系统缓存
*     如果用户的浏览器缓存中没有，浏览器会查找操作系统缓存中是否有这个域名对应的DNS解析结果。其实操作系统也会有一个域名解析的过程，在Windows中可以通过
*   C:\Windows\System32\drivers\etc\hosts文件来设置，你可以将任何域名解析到任何能够访问的IP地址。如果你在这里指定了一个域名对应的IP地址，那么浏览器会
*   首先使用这个IP地址。
* 3.查找路由器缓存
*     如果系统缓存中也找不到，那么查询请求就会发向路由器，它一般会有自己的DNS缓存
* 4.查找ISP DNS 缓存
*     若路由器缓存未命中,则查询ISP DNS 缓存服务器。在我们的网络配置中都会有"DNS服务器地址"这一项，操作系统会把这个域名发送给这里设置的DNS，也就是本地区
*   的域名服务器，通常是提供给你接入互联网的应用提供商。这个专门的域名解析服务器性能都会很好，它们一般都会缓存域名解析结果。大约80%的域名解析都到这里就
*   已经完成了，所以ISP DNS主要承担了域名的解析工作。
* 5.递归搜索
*     若前面的办法都未命中DNS缓存,则:
...
* websocket：支持websocket协议的客户端和服务器能够使用websocket协议进行双向通信，也就是客户端可以随时向服务器发送请求，服务器也能够随时向客户端发送请求
  ...
  （5）建立了websocket连接后，只要客户端和服务器端任意一端不主动断开连接前，通信行为都是在一个持久连接上发起，后续数据与请求都通过帧序列的形式进行传输。
  其他特点包括：
  （1）建立在 TCP 协议之上。
