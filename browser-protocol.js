...
/*
* DNS解析过程:
* 1.当用户在浏览器输入一个域名的时候，最先浏览器会从自己中的缓存中寻找指定的结果。如果找到了域名对应的的ip则域名解析完成
* 2.如果在浏览器中的缓存没有命中，则会在系统的缓存中来查找是否有这个域名对应的DNS解析结果，如果有则域名解析完成。
* 3.系统缓存中未命中之后会把这个域名提交到指定的本地DNS服务器(这个域名解析服务器缓存了大量的域名的DNS解析结果,它的性能较好，
    物理上的距离又比较近，会在很短的时间内返回指定域名的解析结果。)若命中，域名解析完成。
* 4.如果本地DNS服务器未命中，则根据本地DNS服务器的设置（是否设置转发器）进行查询，如果未用转发模式，
    本地DNS就把请求发至13台根DNS，根DNS服务器收到请求后会判断这个域名(.com)是谁来授权管理，并会返回一个负责该顶级域名服务器
    的一个IP。本地DNS服务器收到IP信息后，将会联系负责.com域的这台服务器。这台负责.com域的服务器收到请求后，如果自己无法解析，
