...
/*
 react 的虚拟dom是怎么实现的?
先说说为什么要使用Virturl DOM，因为操作真实DOM的耗费的性能代价太高，所以react内部使用js实现了一套dom结构。
在每次操作真实dom之前，使用实现好的diff算法，对虚拟dom进行比较，递归找出有变化的dom节点，然后对其进行更新操作。
...
2.如果真的发生跨层级移动，如某个DOM及其子节点进行移动挂到另一个DOM下时，React并不知道子树仅仅是发生了移动，而是会直接销毁，并重新创建这个子树，然后再挂在到目标DOM上。
在实现自己的组件时，保持稳定的DOM结构会有助于性能的提升。React官方也不建议做跨层级的操作。在实际使用中，可以通过CSS隐藏或显示某些节点，而不是真的移除或添加DOM节点。
* */
